...
#when we have more than 10 scenarioes which use the same pre-conditions like given or when
# so at that time instead of writing 10 times and we can use background key word and run it
  #when ever we run the scenarioes background keyword runs first andthe scenario runs.
