...
public class AppIndependentMethod{
    ...
    public String getPropValueByKey(String keyName){
        ...
        try{
            fin = new FileInputStream(System.getProperty("user.dir")+"\\TestData\\Locators.properties");
            prop = new Properties();
            prop.load(fin);
    ...
    /******************************************************************
     * Method Name      : createAndReturnBy()
     * purpose          :
     * Author           : sgTester1
     * Reviewed By      : sgTester2
     * Parameters       : browserName
     ******************************************************************/
    public By createAndReturnBy(String locatorNameKey){
        By objBy = null;
        String arrValues[];
        try{
            arrValues = locatorNameKey.split("#");
            switch(arrValues[0].toLowerCase()){
                case "id":
                    objBy = By.id(arrValues[1]);
                    break;
                case "name":
                    objBy = By.name(arrValues[1]);
                    break;
                case "xpath":
                    objBy = By.xpath(arrValues[1]);
                    break;
                case "css":
                    objBy = By.cssSelector(arrValues[1]);
                    break;
                default:
                    System.out.println("Invalid object locator name '"+arrValues[0]+"'");
